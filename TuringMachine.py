...
class MachineTape:
    ...
    def move(self, check_char, changeto_char, direction):
        ...
        if check_char != self.tape[self.pos]:
            raise MachineTapeException(
                "Tape head doesn't match head character")
...
class TuringMachine:
    ...
    def step(self):
        """ Steps 1 - 3 """
        if self.lenStr == 0 and self.state in self.fstates:
            raise TuringAcceptException
        if self.state in self.fstates:
            raise TuringAcceptException
        ...
        """ Steps 4 and 5 """
        head = self.tape.read()
        if head not in self.program[self.state].keys():
            raise TuringErrorException
...
if __name__ == "__main__":
    ...
    #State 2
    m.addTransition(2, '0', 2, 'X', 'R')
    m.addTransition(2, '1', 2, 'X', 'R')
    m.addTransition(2, 'X', 2, 'X', 'R')
    m.addTransition(2, '_', 3, '_', 'L')

    #State 3
    m.addTransition(3, '0', 3, 'X', 'R')
    m.addTransition(3, '1', 3, 'X', 'R')
    m.addTransition(3, 'X', 3, 'X', 'R')
    m.addTransition(3, '_', 4, '_', 'L')
